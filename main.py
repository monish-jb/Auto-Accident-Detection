...
def draw_overlay(frame, tracks, is_accident, details):
    for track in tracks:
        ...
        cv2.putText(
            frame, f"ID {track['id']} {track['class_name']}",
            (x1, max(0, y1 - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2
        )

    if is_accident:
        cv2.putText(
            frame, "ACCIDENT DETECTED", (30, 50),
    ...
    try:
        while True:
            ...
            if is_accident:
                ...
                print(f"[ALERT] Accident detected! score={details['total_score']} "
                      f"involved={details['involved_ids']}")
